...
public class redSmoothAgent : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        RotateTowardsPlayer();
        //if (Vector3.Distance(gameObject.GetComponentInChildren<Transform>().transform.position, playerShip.transform.position) > 10f)  ///////////
        //{

        //SetDestinationPlus(new Vector3(playerShip.transform.position.x, 0, playerShip.transform.position.z));
        // Check if the agent is active and on the NavMesh
        if (Agent.isActiveAndEnabled && Agent.isOnNavMesh)
        {
            // Set the destination only when the agent is ready
            if (Vector3.Distance(gameObject.transform.position, playerShip.transform.position) > 15f)
            {
    ...
    public void SetDestinationPlus(Vector3 Position)
    {
        ...
        {
            StartCoroutine(WaitForNavMeshAndSetDestination(Position));
        }
        else
        {
            Agent.ResetPath();
    ...
    IEnumerator WaitForNavMeshAndSetDestination(Vector3 Position)
    {
